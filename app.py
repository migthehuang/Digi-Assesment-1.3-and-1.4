...
def borrow(book_id):
    ...
    if request.method == 'POST':
        ...
        conn=get_db_connection()
        borrowerid=conn.execute('SELECT * from borrowers')
        borrower_request=conn.execute('INSERT INTO borrowed_books(date_borrowed,books_idbooks, borrowers_idborrowers) VALUES(?,?,?)',((date)),((book_id), (borrowerid.idborrowers)))
        conn.commit() 
        conn.close()   
